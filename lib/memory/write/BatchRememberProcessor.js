...
export class BatchRememberProcessor {
   ...
   * 충돌 키는 (key_id, content_hash)이고 key_id는 배치 전체에서 고정이므로
   * content_hash만으로 접는다.
   *
   ...
   */
  _collapseHashDuplicates(validFragments) {
    const primaryByHash = new Map();
    const unique        = [];
    const aliases       = [];

    for (const item of validFragments) {
      const hash = item.fragment.content_hash;
  ...
  async _insertInTransaction(validFragments, results, ctx) {
    ...
    try {
       ...
       */
      const { unique, aliases } = this._collapseHashDuplicates(validFragments);
