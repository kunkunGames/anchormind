...
const RERANK_INPUT_LIMIT = parsePositiveInt(process.env.MEMENTO_RERANK_INPUT_LIMIT, 30);
const RERANK_TOP_K       = parsePositiveInt(process.env.MEMENTO_RERANK_TOP_K, 15);
...
export class FragmentSearch {
  ...
  async _buildTextRRF(sq, l1Ids, l1MissIds, l1IsFallback, cached, agentId, keyId, timeRange, searchPath, layerLatency) {
    ...
    /** L4: Cross-Encoder Reranker (RRF 상위 후보 정밀 재정렬) */
    if (isRerankerAvailable() && sq.text && combined.length > 0) {
      const rerankerInput = combined.slice(0, RERANK_INPUT_LIMIT);
      ...
      const reranked      = await rerank(
        rerankQuery,
        rerankerInput,
        Math.min(RERANK_TOP_K, RERANK_INPUT_LIMIT)
      ).catch(() => null);
