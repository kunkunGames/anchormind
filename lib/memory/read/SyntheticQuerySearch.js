...
export function selectNewCandidates(aggregated, existingIds, limit) {
  return [...aggregated.entries()]
    ...
    .sort(byDescendingScore(candidate => candidate.similarity))
    .slice(0, limit)
    .map(({ id, similarity }) => ({ id, similarity }));
}

/** ANY hydration 결과에 similarity를 복원하고 공통 동점 정렬을 적용한다. */
export function rankHydratedSyntheticRows(rows, simById) {
  return rows
    .map(row => ({
      ...row,
    ...
    .sort(byDescendingScore(fragment => fragment.similarity ?? 0));
}
 ...
 * @param {boolean|null}  [opts.isAnchor] - true=앵커, false=비앵커, null/미지정=혼합
 * @param {boolean}       [opts.includeSuperseded] - 만료 파편 포함 여부
...
export async function searchSyntheticQueries(vec, opts = {}) {
  ...
  const workspaceFilter = workspaceCondition(params, opts, "f.workspace");
  const workspaceClause = workspaceFilter ? ` AND ${workspaceFilter}` : "";
  const anchorClause = isAnchor === undefined
    ? ""
    : ` AND f.is_anchor = $${params.push(isAnchor)}`;
  const validClause = opts.includeSuperseded === true ? "" : " AND f.valid_to IS NULL";
  ...
  const fragAnchorClause = isAnchor === undefined
    ? ""
    : ` AND f.is_anchor = $${fragParams.push(isAnchor)}`;
  const fragValidClause = opts.includeSuperseded === true ? "" : " AND f.valid_to IS NULL";
  ...
  try {
    ...
    return rankHydratedSyntheticRows(rows, simById);
  } catch (err) {
