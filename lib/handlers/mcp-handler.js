 ...
 * (2) 클라이언트가 직접 전송한 _keyId/_groupKeyIds/_sessionId/_permissions/_defaultWorkspace/_isMaster를
...
export function injectSessionContext(msg, ctx) {
  ...
  delete msg.params.arguments._isMaster;
  ...
  msg.params.arguments._isMaster          = ctx.sessionIsMaster === true;
  ...
  if (!validation.valid) {
    ...
    if (isRecoverable) {
      ...
      if (authResult.valid) {
        ...
        const sessionIsMaster         = authResult.isMaster === true;
        ...
        logInfo(`[Streamable] Session recovered with same-id: ${sessionId}`);

        return { done: false, sessionKeyId, sessionGroupKeyIds, sessionPermissions, sessionDefaultWorkspace, sessionMode, sessionIsMaster };
      } else {
    ...
    let sessionIsMaster         = session.isMaster === true;

    /**
     * 이 필드가 도입되기 전에 Redis에 저장된 세션은 master 여부가 없다.
     * keyId=null만으로는 master와 일부 OAuth 세션을 구분할 수 없으므로 추론하지
     * 않고, 현재 요청의 인증정보가 세션 key와 일치할 때만 권한을 재앵커링한다.
     * 인증정보가 없는 기존 세션은 일반 권한으로 유지해 fail-closed한다.
     */
    if (typeof session.isMaster !== "boolean") {
      const currentAuth = await validateAuthentication(req, msg);
      const currentKeyId = currentAuth.keyId ?? null;
      if (currentAuth.valid && currentKeyId === sessionKeyId) {
        sessionIsMaster = currentAuth.isMaster === true;
        session.isMaster = sessionIsMaster;
        if (REDIS_ENABLED) {
...
async function _createInitializeSession(req, res, msg) {
  ...
  const sessionIsMaster         = authCheck.isMaster === true;
  ...
  if (!reusedExisting) {
    sessionId = await createStreamableSession(
      true, sessionKeyId, sessionGroupKeyIds, sessionPermissions,
      sessionDefaultWorkspace, sessionMode, sessionIsMaster
    );
    recordSessionRecovery("new_session");
    logInfo(`[Streamable] Authenticated session created: ${sessionId}`);
  ...
  return { done: false, sessionId, sessionKeyId, sessionGroupKeyIds, sessionPermissions, sessionDefaultWorkspace, sessionMode, sessionIsMaster };
}
