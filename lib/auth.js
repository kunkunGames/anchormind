...
export function buildAuthDecision(accessKey, authDisabled, bearerToken) {
  ...
  if (!accessKey && authDisabled) {
    return { valid: true, authDisabled: true, keyId: null, isMaster: true };
  }

  /** master 키 직접 비교 */
  if (bearerToken && safeCompare(bearerToken, accessKey)) {
    return { valid: true, keyId: null, isMaster: true };
  }
...
export async function validateAuthentication(req, msg) {
  ...
  if (!ACCESS_KEY) {
    ...
    return decision.valid
      ? { valid: true, keyId: null, groupKeyIds: null, isMaster: true }
  ...
  if (legacyKey && safeCompare(legacyKey, ACCESS_KEY)) {
    return { valid: true, keyId: null, groupKeyIds: null, isMaster: true };
  }
