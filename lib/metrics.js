...
/** 백그라운드 게이트가 현재 점유 중인 Primary 풀 슬롯 수 */
export const backgroundGateActive = new prometheus.Gauge({
  name     : "mcp_background_gate_active_slots",
  help     : "Primary pool connections currently held by background jobs",
  registers: [register]
});

/** 백그라운드 게이트 슬롯을 기다리는 작업 수 */
export const backgroundGateWaiting = new prometheus.Gauge({
  name     : "mcp_background_gate_waiting",
  help     : "Background jobs queued for a primary pool slot",
  registers: [register]
});

/** 백그라운드 게이트 슬롯 상한 */
export const backgroundGateCapacity = new prometheus.Gauge({
  name     : "mcp_background_gate_capacity",
  help     : "Maximum primary pool connections background jobs may hold at once",
  registers: [register]
});
 ...
 * 백그라운드 게이트 지표 갱신
 *
 * @param {{active: number, waiting: number, capacity: number}} stats
 */
export function recordBackgroundGateStats(stats) {
  backgroundGateActive.set(stats.active     || 0);
  backgroundGateWaiting.set(stats.waiting   || 0);
  backgroundGateCapacity.set(stats.capacity || 0);
}
