...
import { updateSessionCounts, recordBatchPoolStats, recordBackgroundGateStats } from "./metrics.js";
...
import { getPrimaryPool, getBatchPool, getPoolStats } from "./tools/db.js";
...
export function startSchedulers({ globalEmbeddingWorkerRef }) {
  /** 세션 정리 (5분) */
  setInterval(() => runInBackground("sessionCleanup", cleanupExpiredSessions), 5 * 60 * 1000);
  setInterval(() => runInBackground("oauthCleanup", cleanupExpiredOAuthData), 5 * 60 * 1000);
  ...
  setInterval(() => {
    try {
      ...
      recordBackgroundGateStats(getPoolStats().background);
    } catch (err) {
  ...
  setInterval(() => runInBackground("consolidate", async () => {
  ...
  setInterval(() => runInBackground("embeddingBackfill", async () => {
    const reg = getSchedulerRegistry();
    try {
  ...
  setInterval(() => runInBackground("morphemeBackfill", async () => {
    const reg = getSchedulerRegistry();
    try {
  ...
  runInBackground("utilityBaseline", () => refreshUtilityBaseline()).catch(err => {
    logWarn(`[Startup] utility baseline refresh failed: ${err.message}`);
  });

  /** Redis 인덱스 웜업 — cold start L1 miss 감소 (비차단) */
  runInBackground("redisWarmup", () => getFragmentIndex().warmup(getPrimaryPool()))
  ...
  import("./memory/embedding/EmbeddingWorker.js")
    ...
    .then(async (worker) => {
      ...
      worker.on("embedding_ready", ({ fragmentId }) => runInBackground("graphLinker", async () => {
        const reg = getSchedulerRegistry();
        try {
