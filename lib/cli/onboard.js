 ...
 *
 * 범위(CLIENT_SPEC v2.1): 환원 불가능한 델타만 설치한다.
 *   C1 (L0 등록)  : Claude `claude mcp add --transport http`; Codex `[mcp_servers.memento]` 멱등 삽입
 *   C2            : no-op — always-on 운영 규칙은 서버 `instructions` 필드가 접속 시 전 클라이언트에 주입(재발명 금지)
 *   C7 (검증 전용): 공급 키로 memory_stats/recall를 호출해 키·그룹 resolution을 assert. 키/그룹을 생성하지 않는다
 *
 * 보류(미구현, 미래 opt-in): C3/C4/C5 훅(SessionStart context / UserPromptSubmit recall /
 *   SessionEnd reflect). 외부 CLI 훅 계약이 버전마다 달라(§14) 본 킷은 templates/·hooks/ 디렉터리를
 *   생성하지 않는다. 훅은 계약 고정 후 별도 플래그로 추가한다.
 *
 * 보안(§12): 마스터 키(MEMENTO_ACCESS_KEY)를 절대 읽지 않는다. 키는 argv가 아니라 환경변수
 *   (--key-env <NAME>)로만 받는다(값이 프로세스 목록·셸 이력에 남지 않도록). MEMENTO_ACCESS_KEY를
 *   가리키거나 그 값과 같으면 거부한다. 키 평문을 stdout에 출력하지 않는다. Claude는 resolved bearer를
 *   ~/.claude.json에 저장하므로 per-platform 키만 쓰고 파일 권한을 제한한다.
 *
...
export const usage = [
  ...
  "  --key-env <NAME>          REQUIRED. env var NAME holding the per-platform key (NEVER the master key).",
  "                            Read from the environment, never from argv. Must NOT be MEMENTO_ACCESS_KEY.",
  "                            Claude persists the resolved bearer in ~/.claude.json — use a per-platform",
  "                            key and restrict file perms. (Raw --key <value> is rejected for security.)",
  ...
  "",
  "Examples:",
  "  export MEMENTO_CLAUDE_KEY=<per-platform key>",
  "  memento-mcp onboard --endpoint 127.0.0.1:57332 --key-env MEMENTO_CLAUDE_KEY --claude",
  "  memento-mcp onboard --endpoint memento.example.net:57332 --key-env MEMENTO_CODEX_KEY --codex",
...
/**
 * per-platform 키를 해석한다. 보안상 값은 argv가 아니라 환경변수(--key-env <NAME>)로만 받는다.
 * 마스터 키 변수(MEMENTO_ACCESS_KEY)를 가리키거나 그 값과 같으면 거부한다(§12).
 *
 * @param {object} args
...
function resolveKey(args) {
  ...
  if (envName === "MEMENTO_ACCESS_KEY") {
    throw new Error(
      "--key-env로 마스터 키 변수(MEMENTO_ACCESS_KEY)를 가리킬 수 없습니다. per-platform 키 변수를 쓰세요(§12)."
    );
  }

  const value = process.env[envName] ?? null;
  if (value && process.env.MEMENTO_ACCESS_KEY && value === process.env.MEMENTO_ACCESS_KEY) {
    throw new Error(
      `환경변수 ${envName} 값이 마스터 키(MEMENTO_ACCESS_KEY)와 동일합니다. per-platform 키만 사용하세요(§12).`
    );
  }
...
function registerClaude(ctx) {
  // 헤더 값: env 이름이 있으면 셸 보간(평문 미저장)을, 직접 값이면 그 값을 사용한다.
  const headerKeyExpr = ctx.keyEnvName
    ? (process.platform === "win32" ? `%${ctx.keyEnvName}%` : `$${ctx.keyEnvName}`)
    : ctx.keyValue;

  const headerArg = `Authorization: Bearer ${headerKeyExpr}`;
 ...
 * 키/그룹을 생성하지 않는다(§6.2). 그룹 멤버십 자체는 어떤 도구 응답에도 노출되지 않으므로
 * (groupKeyIds는 내부 쿼리 스코핑 값) 여기서 단정하지 않는다 — 그룹 배정 검증은 마스터 키를
 * 가진 운영자가 provision-keys에서 수행한다. 검증 실패는 경고로 남기되 설치를 실패시키지 않는다.
 *
...
async function assertKeyGroup(ctx) {
  ...
  try {
    ...
    console.log(
      "[assert] 키 인증 OK — 서버 연결·키 resolution 정상. " +
      "(그룹 멤버십은 운영자가 provision-keys 단계에서 검증합니다.)"
    );
  } catch (err) {
...
export default async function onboard(args) {
  ...
  if (profile === "project") {
    // project 프로파일은 설치형 훅(C3+) 도입 시 의미를 갖는다. 현재 킷은 등록 위주라
    // CLI별 user-scope 등록을 사용하므로 안내만 남긴다.
    console.warn("[onboard] --profile project: 현재 킷은 user-scope 등록만 수행합니다(프로젝트 훅은 C3+ 보류).");
  }
  ...
  if (!uninstall) {
    ...
    if (!keyEnvName) {
      console.error("--key-env <NAME>이 필요합니다(운영자 발급 per-platform 키를 담은 환경변수 이름; 마스터 키 금지).");
      process.exit(2);
    }
  }
