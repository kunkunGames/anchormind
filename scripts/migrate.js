...
async function migrate() {
  ...
  try {
    ...
    const alignSyntheticQueryTable = async () => {
      // Keep the pgvector opclass visible when the extension is installed outside public.
      await client.query(searchPathSQL);
      const result = await alignSyntheticQueryEmbedding(client);
      if (result.action === "converted") {
        console.log(
          `Synthetic-query embedding aligned: ${result.targetType} -> ${result.sourceType}; ` +
          "derived rows were deleted; eligible fragments can be regenerated by SyntheticQueryWorker.backfill()."
        );
    ...
    if (pending.length === 0) {
      ...
      await alignSyntheticQueryTable();
      return;
    }
...
migrate().catch(err => {
  // Log the Error object so nested causes retain PostgreSQL messages and codes.
